...
const chunkDelayMs = Number(process.env.TTS_CHUNK_DELAY_MS || process.env.EDGE_TTS_DELAY_MS || (provider === 'edge' ? 8000 : 0));
const retryDelayMs = Number(process.env.TTS_RETRY_DELAY_MS || (provider === 'edge' ? 30000 : 1500));
...
function runCommand(command, commandArgs, label, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      finish(new Error(summarizeCommandError(label, code, stderr)));
    });
  });
}
...
function summarizeCommandError(label, code, stderr) {
  ...
  if (isEdgeAccessError) {
    return [
      ...
      'This usually means the unofficial Edge Read Aloud endpoint is rate-limited or blocked for this network/session.',
      'Try again later, increase TTS_CHUNK_DELAY_MS, reduce TTS_CHUNK_CHARS, or use TTS_PROVIDER=piper.',
    ].join(' ');
  }
